...
public class UsuariosDAO 
{
	public List<UsuariosDTO> cargaUsuarios(Connection objConnection) throws SQLException 
	{
		...
		ResultSet rs = null;
		try {
			ps = objConnection.prepareStatement(" select * from tb_usuarios ");
	...
	public List<UsuariosDTO> cargaUsuariosDescripcion(Connection objConnection, String strUsuario) throws SQLException 
	{
		...
		ResultSet rs = null;
		try {
			ps = objConnection.prepareStatement(" select * from tb_usuarios where id_usuario like ? or nombres like ? or usuario like ? ");
	...
	public int insertaUsuarioDAO(Connection objConnection, String NomApellidos, String strUser, String strPassword) throws SQLException 
	{
		/*** ***/
		PreparedStatement ps = null;
		ResultSet rs = null;
		...
		try {
			...
			ps = objConnection.prepareStatement("insert into tb_usuarios (id_usuario,nombres,usuario, password)values(?, ?, ?, ?)");
			ps.setInt(1, id);
	...
	public int idMayor(Connection objConnection) throws SQLException 
	{
		...
		try {
			ps = objConnection.prepareStatement("select ifnull(max((id_usuario))+1,-1) as idMax from tb_usuarios;");
			rs = ps.executeQuery();
	...
	public int actualizaUsuarioDAO(Connection objConnection, int idUser, String NomApellidos, String strUser, String strPassword) throws SQLException 
	{
		...
		try 
		{
			...
			ps = objConnection.prepareStatement("update tb_usuarios x set x.nombres = ?, x.usuario = ?, x.password= ?"
												+ "where x.id_usuario = ?");
